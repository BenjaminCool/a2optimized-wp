...
class A2_Optimize_WP {

    public $phpversion = '5.3';

    public function __construct() {
        if(version_compare(phpversion(), $this->phpversion) < 0) {
        ...
        elseif(!file_exists('/opt/a2-optimized/wordpress_encoded/a2-optimized.php')){
            add_action('admin_notices', array( $this, 'files_not_found_notice'));
        }
        else{
    ...
    public function files_not_found_notice(){
  ...
  echo "<div class='error'><br>{$message}<br></div>";
    ...
    echo '<div class="updated fade">' .
      __('Error: plugin "A2 Optimized" requires a newer version of PHP to be running.',  'a2-optimized').
            '<br/>' . __('Minimal version of PHP required: ', 'a2-optimized') . '<strong>' . $this->phpversion . '</strong>' .
...
$atwo_optimize_wp = new A2_Optimize_WP();